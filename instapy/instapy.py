...
from .util import get_active_users_commenting
...
from .unfollow_util import follow_given_user_commenters
...
class InstaPy:
    ...
    def set_selenium_local_session(self):
        ...
        self.logger.critical('Session started - %s'
    ...
    def interact_user_commenters2(self,
                              usernames,
                              amount_of_follows=10,
                              fpm = 60):
        ...
        followed_total = 0
        total_liked_img_total = 0
        already_liked_new_total = 0
        inap_img_total = 0
        commented_total = 0


        for user in usernames:
            commenters = get_active_users_commenting(self.browser, user, 5, self.logger)
            finalCommenters = []

            if len(commenters) < amount_of_follows:
                self.logger.info('Reducing number of commenters to {}'.format(len(commenters)))
                amount_of_follows = len(commenters)
                finalCommenters = commenters
            else:
                sample = random.sample(range(0, len(commenters)), amount_of_follows)
                for num in sample:
                    finalCommenters.append(commenters[num])


            self.set_do_follow(enabled=True, percentage=100)
            self.set_do_like(enabled=True, percentage=50)

            followed_new, total_liked_img_new, already_liked_new, inap_img_new, commented_new = self.interact_by_users(finalCommenters, amount=2, randomize=False, media=None)

            followed_total += followed_new
            total_liked_img_total += total_liked_img_new
            already_liked_new_total += already_liked_new
            inap_img_total += inap_img_new
            commented_total += commented_new

        self.logger.info('Total run followed: {}'.format(followed_total))
        self.logger.info('Total run liked: {}'.format(total_liked_img_total))
        self.logger.info('Total run already Liked: {}'.format(already_liked_new_total))
        self.logger.info('Total run innapropriate: {}'.format(inap_img_total))
        self.logger.info('Total run commented: {}'.format(commented_total))

        return self


    def interact_user_commenters(self,
                              usernames,
                              amount=10,
                              randomize=False,
                              interact=False,
        ...
        for user in usernames:
            ...
            try:
                userFollowed += follow_given_user_commenters(self.browser,
                                                            self.dont_include,
                                                            user,
                                                            amount,
                                                            self.follow_restrict,
    ...
    def end(self):
        ...
        self.logger.critical('Session ended - {}'.format(
                datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
        self.logger.critical('-' * 20 + '\n\n')
