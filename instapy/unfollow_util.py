...
def follow_given_user_commenters(browser,
                      dont_include,
                      user_name,
                      amount,
                      follow_restrict,
                      ...
                      logfolder,
                      follow_times):
    commenters = get_active_users_commenting(browser, user_name, 5, logger)
    followed_list =[]
    hasSlept=False;
    followNum = 0;
    real_amount = amount;

    if len(commenters) < amount:
        logger.info('Reducing number of commenters to {}'.format(len(commenters)))
        amount = len(commenters)

    if randomize:
        sample = random.sample(range(0, len(commenters)), amount)
        finalCommenters = []
        for num in sample:
    ...
    for acc_to_follow in finalCommenters:
        if followNum >= real_amount:
            logger.info("--> Total followNum reached: {}"
                        .format(followNum))
            break

        if(acc_to_follow not in dont_include and follow_restrict.get(acc_to_follow, 0) < follow_times):
            ...
            if followNum != 0 and hasSlept is False and followNum % 10 == 0:
                ...
                sleep(randint(delay-10, delay+10))
                hasSlept = True
                continue
            ...
            try:
                ...
                followed_list.append(acc_to_follow)
                delayint = randint(5,20)
                sleep(delayint)
            except NoSuchElementException:
