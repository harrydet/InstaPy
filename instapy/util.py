...
def get_active_users_commenting(browser, username, posts, logger):
    """Returns a list with users who commented on the latest posts"""
    ...
    for count in range(1, posts):
        ...
        #logger.info('List: {}'.format(tmp_list))
        try:
            if len(tmp_list) is not 0:
                for user in tmp_list:
                    active_users.append(user.text)
        except:
            logger.error('User disappeared!')
